...
const TaskProject: React.FC<{}> = ({}) => {
  ...
  const loadDataToTask = (data) => {
    ...
    data.forEach(element => {
      ...
      switch(element.typeTask) {
        ...
        case "Complete": complete.push(tmp); break;
      }
    });
  ...
  const deleteTask = (columnId, itemId, columns, setColumn) => {
    ...
    taskService.deleteTask({id: itemId, projectId: projectId})
    .then((res) => {
      socket.emit("addTask", {listTask: res.data.data, roomId: projectId});
    });
  }
  const onDragEnd = (result, columns, setColumns) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      let data = {id: columns[source.droppableId].items[source.index].id, 
                  projectId: projectId,
  ...
  return (
    ...
    <ModalCreateTask
      show = {showCreateTask}
      funcQuit = {()=>{setShowCreateTask(false)}}
      defaultName=""
      listUser = {listUser}
      projectId = {projectId}
      funcCreate = {async (data) => {await createTaskPlanned(data); }}
    ></ModalCreateTask>
                            ...
                            {column.items.map((item, index) => {
                              return (
                                <Draggable
                                  ...
                                  draggableId={item.id}
                                  index={index}
                                  isDragDisabled={userId===item.authorId? false: true}
                                >
                                  {(provided, snapshot) => {
                                    return (
                                      <div
                                          ...
                                          >
                                            <div className="card-body-task">
                                              <div className="row no-gutters align-items-center content-name-tasks  ">
                                                ...
                                                <UncontrolledDropdown
                                                  disabled={item.authorId === userId? false: true}
                                                >
                                                  <DropdownToggle
                                                    ...
                                                    disabled={item.authorId === userId? false: true}  
                                                  >
                                                    <i className={item.authorId === userId?"fas fa-ellipsis-v text-info":"fas fa-ellipsis-v "} />
                                                  </DropdownToggle>
                                                  <DropdownMenu className="dropdown-menu-arrow" right>
                                                    <DropdownItem
                                                      ...
                                                      <span style={{fontWeight:"bold", color: "gray"}}>Edit</span>
                                                    </DropdownItem>
                                                    <DropdownItem
                                                      href="#pablo"
                                                      onClick={(e) => {
                                                        // deleteTask(columnId, item.id, tasks, setTasks);
                                                        setDataTaskDelete({columnId: columnId, itemId: item.id});
                                                        setShowDelete(true);
                                                      }} >
