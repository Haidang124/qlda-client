...
import {
  Container, DropdownItem, DropdownMenu, DropdownToggle, UncontrolledDropdown,
} from 'reactstrap';
...
const MyCalendar: React.FC = () => {
    const [dataUser, setDataUser] = useState({
      ...
      const [data, setData] = useState({
        ...
        arrTaskId: {},
        listTask: {
          ['']: {id:"", authorId: "", taskname: "", typeTask: "", projectId: "", projectName: "", deadline:""}
        },
        statusDate: {
          ['']: {
            finished: 0,
      ...
      useEffect(() => {
        ...
        taskService.getAllTaskUser().then((res) => {
      ...
      const getAllDay = () => {
        ...
        for(let j=0; j < Math.ceil((numOfDays+firstDay+1)/7); j++) {
          let dayOfWeek = [];
          for(let i = 0; i < 7; i++) {
            let date = new Date(monthYear.year, monthYear.month, j*7+i-firstDay);
            dayOfWeek.push({
              fullDate: date,
              day: date.getDay(),       //Thứ
                  ...
                  <div className="row">
                    <div className="col-2">
                      <div className="btn btn-outline-primary"
                        onClick={(e) => {
                          let newDate = new Date(monthYear.year, monthYear.month-1,1);
                          setMonthYear({
                    ...
                    <div className="col-2 d-flex justify-content-center">
                      <UncontrolledDropdown disabled={false}>
                        ...
                        <DropdownMenu className="dropdown-menu-arrow" left>
                          {Array.from({length: 12}, (_, index) => index).map((value, index) => {
                            return (
                              <DropdownItem
                                onClick={(e) => {
                                  setMonthYear({
                    ...
                    <div className="col-2 d-flex justify-content-center">
                      <UncontrolledDropdown disabled={false}>
                        <DropdownToggle
                          ...
                          disabled={false}
                          onClick={(e) => {
                            let item = document.getElementById("year-"+monthYear.year);
                            item.scrollIntoView();
                          }}>
                          {monthYear.year}
                        </DropdownToggle>
                        ...
                        >
                          {Array.from({length: 131}, (_, index) => index+1970).map((value, index) => {
                            return (
                ...
                <div className="col-4">
                  <div className="row d-flex justify-content-center" style={{height:"100%"}}>
                    <div className="col-10" >
                      <UncontrolledDropdown disabled={false} style={{width:"100%", height:"100%"}}>
                        <DropdownToggle
                          ...
                          disabled={false}
                          onClick={(e) => {}}>
                          {isNull(selectDateTask)? "Chọn ngày deadline" : (
                            <>
                              {formatDate(new Date(selectDateTask))+"  "}
                              <span className="text-danger">({data.statusDate[selectDateTask].unfinished}) </span>
                              <span className="text-success">({data.statusDate[selectDateTask].finished}) </span>
                            </>
                          )}
                        </DropdownToggle>
                        <DropdownMenu className="dropdown-menu-arrow" left 
                        ...
                        >
                          {Object.entries(data.arrDate).length === 0 ? (
                            ...
                            <>
                              {Object.entries(data.arrDate).map(([dateTime, arrTaskId], index) => {                           
                                return (
                                  <DropdownItem
                                    onClick={(e) => {
                                    ...
                                    <span style={{color:"#c1c1c1"}}><b>
                                      {formatDate(new Date(dateTime))}{"  "}
                                      <span className="text-danger">({data.statusDate[dateTime].unfinished}) </span>
                                      <span className="text-success">({data.statusDate[dateTime].finished}) </span>
                                    </b></span>
                                  </DropdownItem>
                                )
                              })}
          ...
          <div className="row mt-3 mb-5">
            <div className="col-12">
              <table className="tableCalendar">
                <thead>
                  <th><div className="btn btn-outline-dark" style={{width: "90%"}}>Chủ nhật</div></th>
                  <th><div className="btn btn-outline-info" style={{width: "90%"}}>Thứ hai</div></th>
                  <th><div className="btn btn-outline-info" style={{width: "90%"}}>Thứ ba</div></th>
                  <th><div className="btn btn-outline-info" style={{width: "90%"}}>Thứ tư</div></th>
                  <th><div className="btn btn-outline-info" style={{width: "90%"}}>Thứ năm</div></th>
                  <th><div className="btn btn-outline-info" style={{width: "90%"}}>Thứ sáu</div></th>
                  <th><div className="btn btn-outline-info" style={{width: "90%"}}>Thứ bảy</div></th>
                </thead>
                <tbody>
                  {
                    [...getAllDay()].map((week, i) => {
                      return (
                        <tr>
                          {week.map((date, j) => {
                            if(date.month != monthYear.month) {   // khác tháng
                              return ( <>
                                <td>
                                <button className="btn" style={{border:"1px solid #939393", height: "100%",
                                width:"100%", backgroundColor:"#c9c9c9", color:"white"}}>
                              ...
                              return ( <>
                                <td>
                                  <button className="btn" style={{border:"1px solid #939393", height: "100%",
                                    width:"100%", color: checkCurrentDay(date.fullDate)?"#007bff":"#6c757d"}}
                                    onClick={()=>{
                                      choseDate(date);
                                    }}
                                  >
                              ...
                              else if(equalDate(new Date(date.fullDate), currentDate) >= 1) {   // chưa hoàn thành các task và còn deadline
                                return ( <>
                                  <td>
                                    <button className="btn" style={{border:"2px solid #ffc107", height: "100%",
                                      width:"100%", color: checkCurrentDay(date.fullDate)?"#007bff":"#6c757d"}}
                                      onClick={()=>{
                                        choseDate(date);
                                      }}
                              ...
                              else {    // chưa hoàn thành các task và hết deadline
                                return ( <>
                                  <td>
                                    <button className="btn" style={{border:"2px solid #e22e2f", height: "100%",
                                      width:"100%", color: checkCurrentDay(date.fullDate)?"#007bff":"#6c757d"}}
                                      onClick={()=>{
                                        choseDate(date);
                                      }}
