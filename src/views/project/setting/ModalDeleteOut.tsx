...
const ModalDeleteOut: React.FC<any> = (props: any) => {
    ...
    const deleteProject = () => {
      projectService.deleteProject({projectId: props.project.projectId}).then((res) => {
        ...
        toast.error("Lỗi khi rời/ xóa project!");
      });
    }
    const outProject = () => {
      projectService.deleteMember({projectId: props.project.projectId, memberId: props.userId}).then((res) => {
        toast.success("Rời project thành công!");
        window.location.href = "/admin/index";
      }).catch((err) => {
        toast.error('Lỗi! Tạm thời không thể rời project!')
      })
    }
  return (
    <>
      <Modal
        ...
        <Modal.Header closeButton>
          <Modal.Title>
              <h3>{props.title === 1 ? "XÓA PROJECT" : "RỜI PROJECT"}</h3>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
              ...
              <div className="col-10">
                  <div className="row">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" className="bi bi-exclamation-triangle" viewBox="0 0 16 16">
                        ...
                        <span className="ml-3">Gõ <b>"{props.project.nameProject}"</b> để {props.title === 1? "xóa" : "rời"} project <b>{props.project.nameProject}</b></span>
                  </div>
